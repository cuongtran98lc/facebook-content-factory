...
function buildPrompt(batch: PreparedTarget[]): string {
  ...
  return [
    ...
    '',
    'Quy tắc:',
    '- title: luôn viết bằng tiếng Anh, bắt đầu chính xác bằng "What if " và kết thúc câu hỏi bằng "?" (trước hậu tố Part/Episode nếu có). Đây là ngoại lệ ngôn ngữ chỉ áp dụng cho title; caption và description vẫn theo ngôn ngữ mục tiêu.',
    '- title: biến tình huống, lựa chọn hoặc mâu thuẫn chính của video thành một câu hỏi giả định tự nhiên theo dạng "What if [specific story premise]?". Không chỉ ghép "What if" trước tiêu đề cũ. Không bịa năng lực, sự kiện hoặc mức cược không có trong nội dung; không tiết lộ twist/kết thúc, không lặp nguyên hook, không thêm nhãn tỷ lệ khung hình.',
