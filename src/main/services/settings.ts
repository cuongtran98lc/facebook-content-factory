...
export class SettingsService {
  ...
  return {
   ...
   claudeModel: settings.claudeModel || DEFAULT_SETTINGS.claudeModel,
   hasOpenAIKey: Boolean(settings.openaiApiKeyEncrypted) || Boolean(process.env.OPENAI_API_KEY),
   hasGeminiKey: Boolean(settings.geminiApiKeyEncrypted) || Boolean(process.env.GEMINI_API_KEY),
   hasClaudeKey: Boolean(settings.claudeApiKeyEncrypted) || Boolean(process.env.CLAUDE_API_KEY),
  };
 }
 ...
 saveAI(input: SaveAISettingsInput): AISettingsDTO {
  ...
  settings.claudeModel = input.claudeModel.trim() || DEFAULT_SETTINGS.claudeModel;
  ...
  if (input.clearClaudeKey) delete settings.claudeApiKeyEncrypted;
  else if (input.claudeApiKey?.trim()) settings.claudeApiKeyEncrypted = encrypt(input.claudeApiKey.trim());
 ...
 getVoice(): VoiceSettingsDTO {
  ...
  return {
   ...
   hasElevenLabsKey: Boolean(settings.elevenLabsApiKeyEncrypted) || Boolean(process.env.ELEVENLABS_API_KEY),
  };
 }
 ...
 getElevenLabsApiKey(): string {
  let key = decrypt(readSettings().elevenLabsApiKeyEncrypted);
  if (!key) {
 ...
 getModel(provider: AIProviderName): string {
  ...
  if (provider === 'claude') return settings.claudeModel || DEFAULT_SETTINGS.claudeModel;
 ...
 getApiKey(provider: AIProviderName): string {
  ...
  const encrypted = provider === 'openai'
   ...
   : provider === 'claude'
     ? settings.claudeApiKeyEncrypted
