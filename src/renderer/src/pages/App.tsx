...
import { MindsetContentView } from '../components/MindsetContentView';
...
export default function App() {
 ...
 const [topic, setTopic] = useState(
  'Mẹ chia tài sản cho các con, thử lòng con cái, mâu thuẫn anh chị em, con nuôi vs con ruột, di chúc bí mật và bài học lòng hiếu thảo',
 );
 ...
 useEffect(() => {
  ...
  setVoiceTestText(
   selected?.contentLanguage?.startsWith('en')
    ? 'That day, I returned to the old house, unaware of what awaited me behind the door.'
    : 'Ngày hôm đó, tôi trở về căn nhà cũ và không ngờ điều đang chờ mình phía sau cánh cửa.',
  );
 ...
 async function createProject(event: FormEvent) {
  ...
  setMessage('');
  try {
   const created = await window.contentFactory.projects.create({
    name,
    topic,
    niche,
    targetMarket,
 ...
 async function handleGenerateWithTopic() {
  ...
  try {
   ...
   const currentTopicText =
    topic.trim() || COMPILED_TOPICS.find(t => t.id === selectedTopicId)?.prompt || 'Chuyện gia đình';
   const currentNiche = niche.trim() || 'family';

   if (targetProjectId) {
    setMessage(`Đang cập nhật chủ đề & tạo ${ideaCount} ideas bằng ${settings.provider}...`);
    await window.contentFactory.projects.update(targetProjectId, {
    ...
    const projName =
     name.trim() || `${topicObj ? topicObj.title : 'Project'} ${new Date().toLocaleDateString('vi-VN')}`;
    const created = await window.contentFactory.projects.create({
     name: projName,
     topic: currentTopicText,
     niche: currentNiche,
 ...
 async function handleMindsetScriptImported(script: ScriptDTO) {
  if (!selected) return;
  await loadProjectData(selected.id);
  await reloadProjects();
  setActiveScriptId(script.id);
  setView('scripts');
  setMessage(`Đã tạo script "${script.title}" từ nội dung crawl. Tiếp tục chọn giọng đọc, background và render ở đây.`);
 }
 ...
 async function generateThumbnail(
  ...
  if (engine !== 'BUILTIN_2D') {
   ...
   if (!hasKey)
    return setMessage(
     `Chưa có API key cho ${settings.provider}. Vào Settings để thêm key hoặc chọn "Stickman 2D Vector (Engine nội bộ)".`,
    );
  }
  setBusy(true);
  setMessage(
   engine === 'BUILTIN_2D'
    ? 'Đang tạo thumbnail Google Flow 2D Vector...'
    : `Đang tạo thumbnail Google Flow 2D bằng ${settings.provider}...`,
  );
  try {
   const saved = await window.contentFactory.scripts.update({
    scriptId: activeScript.id,
 ...
 async function generateVideoThumbnail() {
  ...
  try {
   setStoryMedia(
    await window.contentFactory.storyMedia.extractThumbnailFromVideo({
     projectId: selected.id,
 ...
 async function generateStickVideo(source: 'API' | 'CODEX_CLI' | 'CLAUDE_CLI' | 'ANTIGRAVITY_CLI') {
  ...
  if (!isReel) {
   if (editorContent !== activeScript.content)
    return setMessage('Truyện có chỉnh sửa. Hãy Generate Story MP3 lại trước để hoạt hình khớp lời đọc.');
  }
  ...
  try {
   setStoryVideoProgress({
    ...
    percent: 0,
    stage: 'VIDEO',
    message: 'Đang kiểm tra MP3 và FFmpeg hiện tại...',
   });
 ...
 async function regenerateReelThumbnails() {
  ...
  if (typeof window.contentFactory.storyMedia.regenerateReelThumbnails !== 'function') {
   setMessage(
    'Preload Electron đang là bản cũ. Hãy thoát hoàn toàn ứng dụng và chạy lại npm start để dùng Generate lại Thumbnail Reel.',
   );
   return;
  }
  setBusy(true);
  ...
  <div className="app-shell">
   <aside className="sidebar">
    ...
    <nav>
     ...
     <button className={`nav ${view === 'mindset-content' ? 'active' : ''}`} onClick={() => setView('mindset-content')}>
      🧠 Mindset từ Web
     </button>
   ...
   <main className="main">
        ...
        : view === 'ideas'
          ...
          : view === 'scripts'
            ...
            : view === 'stickman-engine'
              ...
              : view === 'mindset-content'
                ? '🧠 Xây dựng nội dung Mindset từ Web'
                : view === 'scheduler'
                  ? '📅 Content Scheduler'
                  : view === 'export-queue'
      ...
      <p>
       {view === 'scripts'
        ...
        : view === 'stickman-engine'
          ...
          : view === 'mindset-content'
            ? 'Crawl 1 bài viết → AI viết kịch bản mindset mới theo 5 pillar/8 công thức tiêu đề → tiếp tục ở Scripts'
       ...
       <div className="topic-generator-fields">
        <label>
         ...
         <select
          ...
          onChange={e => {
           ...
           setContentLanguage(e.target.value === 'VN' ? 'vi-VN' : e.target.value === 'GB' ? 'en-GB' : 'en-US');
          }}>
          {MARKETS.map(item => (
        ...
        {selected && (
         <button
          ...
          Lưu cho project hiện tại
         </button>
        )}
       </div>
       <p className="muted">
        Áp dụng khi tạo project hoặc generate chủ đề. Với project hiện tại, bấm Lưu trước khi tạo nội dung. Nội dung đã
        tạo không tự dịch; hãy generate lại và chọn giọng đọc phù hợp ngôn ngữ.
       </p>
      </section>
      <section className="card compiled-topics-section">
       <div className="topics-gallery-header">
        <div>
         ...
         <p>
          Chọn một chủ đề đã biên soạn bên dưới. Bạn có thể tự chỉnh sửa nội dung và bấm <strong>Generate Ideas</strong>{' '}
          bất cứ lúc nào.
         </p>
        </div>
       ...
       <div className="card project-card">
        ...
        {selected ? (
         <>
          ...
          <div className="meta">
           {selected.niche ?? 'general'} · {selected.status} · {selected.targetMarket ?? 'VN'} /{' '}
           {selected.contentLanguage ?? 'vi-VN'}
          </div>
        ...
        <>
         ...
         {activeScript.type === 'LONG_STORY' && (
          <ThumbnailGenerator
           ...
           onGenerate={(title, concept, engine, includeTextOverlay) =>
            void generateThumbnail(title, concept, engine, includeTextOverlay)
           }
           onExtractFromVideo={(videoPath, timeSeconds) => void extractThumbnailFromVideo(videoPath, timeSeconds)}
          />
         )}
         {activeScript.type === 'LONG_STORY' && (
          <>
           ...
           <div className="voice-gate">
            <div className="voice-title">
             <div>
              ...
              <span>
               {selected?.voiceName ? `Selected: ${selected.voiceName}` : 'Test và chọn giọng trước khi approve'}
              </span>
             </div>
             <button
              className="secondary"
           ...
           <StoryMediaFlow
            projectId={selected?.id}
            onRefreshMedia={() => {
             if (selected?.id) void window.contentFactory.storyMedia.get(selected.id).then(setStoryMedia);
            }}
           ...
           <div className="generate-reels">
            ...
            <div className="button-row grow">
             <button className="primary" onClick={approveAndGenerateReels} disabled={busy || !selected?.voiceId}>
              Approve Version & Generate Reels
             </button>
             <button className="secondary" onClick={generateReels} disabled={busy || !stories.some(s => s.approved)}>
              Generate Again
             </button>
            </div>
           </div>
    ...
    {view === 'mindset-content' && (
     <MindsetContentView
      ...
      selectedProjectId={selectedId}
      onSelectProject={setSelectedId}
      onImported={handleMindsetScriptImported}
     />
    )}
    ...
    {view === 'settings' && (
     <section className="settings-grid">
      <form className="card form" onSubmit={saveSettings}>
       ...
       <label>
        Groq model
        <input value={settings.groqModel} onChange={e => setSettings({ ...settings, groqModel: e.target.value })} />
       </label>
       <label>
        Groq API key
        <input
         type="password"
         placeholder={
          settings.hasGroqKey ? 'Saved — nhập key mới để thay thế' : 'Paste Groq API key (console.groq.com — free)'
         }
         value={groqKey}
       ...
       <label className="check">
        <input type="checkbox" checked={clearGroqKey} onChange={e => setClearGroqKey(e.target.checked)} /> Xóa Groq key
        đã lưu
       </label>
       ...
       <label>
        Gemini model
        <input value={settings.geminiModel} onChange={e => setSettings({ ...settings, geminiModel: e.target.value })} />
       </label>
       <label>
        Gemini API key
        <input
       ...
       <label className="check">
        <input type="checkbox" checked={clearGeminiKey} onChange={e => setClearGeminiKey(e.target.checked)} /> Xóa
        Gemini key đã lưu
       </label>
       ...
       <label>
        OpenAI model
        <input value={settings.openaiModel} onChange={e => setSettings({ ...settings, openaiModel: e.target.value })} />
       </label>
       <label>
        OpenAI API key
        <input
       ...
       <label className="check">
        <input type="checkbox" checked={clearOpenAIKey} onChange={e => setClearOpenAIKey(e.target.checked)} /> Xóa
        OpenAI key đã lưu
       </label>
       ...
       <p className="hint">
        CLI providers dùng Claude Code / Codex / Antigravity CLI đã đăng nhập sẵn trên máy (không cần API key). Cần cài
        đặt CLI tương ứng và đăng nhập trước ở Terminal.
       </p>
      ...
      <div className="stack-cards">
       <div className="card form">
        ...
        <label>
         ...
         <input
          type="password"
          placeholder={voiceSettings.hasElevenLabsKey ? 'Saved — nhập key mới để thay thế' : 'Paste ElevenLabs API key'}
          value={elevenLabsKey}
          onChange={e => setElevenLabsKey(e.target.value)}
         />
        </label>
        <label className="check">
         <input type="checkbox" checked={clearElevenLabsKey} onChange={e => setClearElevenLabsKey(e.target.checked)} />{' '}
