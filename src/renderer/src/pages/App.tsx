...
export default function App() {
 ...
 async function generateThumbnail() {
  ...
  const hasKey = settings.provider === 'openai' ? settings.hasOpenAIKey : settings.provider === 'claude' ? settings.hasClaudeKey : settings.hasGeminiKey;
 ...
 async function renderStoryVideo() {
  ...
  if (!selected.voiceId) return setMessage('Hãy Test voice và chọn Use this voice trước khi tạo video.');
  ...
  setStoryVideoProgress({
   ...
   percent: 0,
   stage: 'STARTING',
   message: 'Đang chuẩn bị âm thanh...',
  });
  setStoryVideoGenerating(true);
  setBusy(true);
  setMessage('Đang lưu và tạo Story MP3 từ giọng đọc (tự động chia chunk nếu story dài)...');

  try {
   // 1. Lưu script mới nhất
   const saved = await window.contentFactory.scripts.update({
    scriptId: activeScript.id,
    title: activeScript.title ?? undefined,
    content: editorContent,
   ...
   // 2. Tạo MP3
   const mediaWithAudio = await window.contentFactory.storyMedia.generateAudio({
    projectId: selected.id,
    scriptId: saved.id,
   });
   setStoryMedia(mediaWithAudio);
   ...
   setStoryVideoProgress({
    ...
    percent: 0,
    stage: 'STARTING',
    message: videoFormat === 'REEL' ? 'Đang tính số video Short 9:16...' : 'Đang render Story video...',
   });
   setMessage(
    videoFormat === 'REEL'
   ...
   setMessage(
    metadataDone
     ? videoFormat === 'REEL'
       ? `✓ Đã tạo xong: ${output?.parts.length ?? 0} video Short 9:16, mỗi phần có SFX + title/description riêng.`
       : '✓ Video đã render xong có SFX + title/description.'
      ...
      <section className="card crawler-first-step">
       <div className="crawler-first-head">
        ...
        <div>
         ...
         <p>
          Dán link truyện; hệ thống sẽ tìm danh sách chương, mở chi tiết từng chương và tạo mỗi chương thành một tập.
         </p>
        </div>
       ...
       {selected ? (
        <>
         ...
         <p className="crawler-note">
          Trang tổng không được dùng làm tập. Mỗi link chương tạo đúng một script/video tập; xong sẽ tự chuyển sang bước
          Scripts.
         </p>
         {crawlProgress && (
    ...
    {view === 'settings' && (
     <section className="settings-grid">
      <form className="card form" onSubmit={saveSettings}>
       ...
       <label>
        Claude model
        <input value={settings.claudeModel} onChange={e => setSettings({ ...settings, claudeModel: e.target.value })} />
       </label>
       <label>
        Claude API key
        <input
         type="password"
         placeholder={settings.hasClaudeKey ? 'Saved — nhập key mới để thay thế' : 'Paste Claude API key'}
         value={claudeKey}
         onChange={e => setClaudeKey(e.target.value)}
        />
       </label>
       <label className="check">
        <input type="checkbox" checked={clearClaudeKey} onChange={e => setClearClaudeKey(e.target.checked)} /> Xóa
        Claude key đã lưu
       </label>
       <button type="button" className="secondary" disabled={busy} onClick={() => testAI('claude')}>
        Test Claude
       </button>
       <button className="primary" disabled={busy}>
        Save settings
