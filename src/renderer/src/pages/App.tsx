...
export default function App() {
  ...
  async function generateStoryMp3() {
    ...
    try {
      ...
      setStoryMedia(media); await reloadProjects(); setMessage('✓ Bước 1/3 hoàn tất: Story MP3 đã sẵn sàng. Tiếp theo hãy Select Video.')
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)) } finally { setBusy(false) }
  }
  ...
  async function chooseBackground() {
    ...
    try {
      ...
      if (media) { setStoryMedia(media); setMessage('✓ Bước 2/3 hoàn tất: Background video đã sẵn sàng. Có thể Generate Story Video.') }
  ...
  async function renderStoryVideo() {
    ...
    if (!storyMedia?.audioPath) return setMessage('Hãy hoàn tất bước 1: Generate Story MP3 trước.')
    if (!storyMedia.backgroundPath) return setMessage('Hãy hoàn tất bước 2: Select Video trước.')
    ...
    try {
      ...
      setStoryMedia(media); await reloadProjects(); setMessage('✓ Bước 3/3 hoàn tất: Story video đã render xong.')
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)) } finally { setBusy(false) }
  }
