...
import type { AIProviderName, AISettingsDTO, AppHealth, BackgroundKind, CrawlProgress, FitMode, IdeaDTO, ProjectDTO, ReelVideoProgress, ScriptDTO, SoundEffectOptions, StoryMediaDTO, StoryVideoProgress, VideoFormat, VoiceDTO, VoiceSettingsDTO } from '../../../shared/types'
...
export default function App() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      setBusy(false)
    })().catch(error => { setReelGenerating(false); setBusy(false); setMessage(error instanceof Error ? error.message : String(error)) })
  }, [])
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        if (!active) return
        setVoices(rows)
        setVoiceId(current => current && rows.some(voice => voice.id === current) ? current : (selected?.voiceId && rows.some(voice => voice.id === selected.voiceId) ? selected.voiceId : rows[0]?.id ?? ''))
      } catch {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setEstimatedProgress(value => {
  ...
  async function importStory() {
    ...
    setBusy(true); setMessage('Đang thêm truyện TXT thành một version mới...')
    try {
      ...
      setImportTitle('')
      setImportContent('')
      setMessage(`✓ Đã nhập truyện thành LONG_STORY v${row.version}. Chọn voice rồi có thể generate MP3/Reels/Video như bình thường.`)
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)) } finally { setBusy(false) }
  }

  async function crawlStory() {
    if (!selected) return
    if (!/^https?:\/\//i.test(storyUrl.trim())) return setMessage('Hãy nhập link HTTP/HTTPS của trang chi tiết truyện.')
    setCrawlProgress({ current: 0, total: crawlEpisodeLimit, percent: 0, stage: 'DISCOVERING', message: 'Đang kết nối website...' })
    setCrawlGenerating(true)
    setBusy(true); setMessage('Đang tìm danh sách chương từ link truyện...')
    try {
      const result = await window.contentFactory.crawler.crawl({ projectId: selected.id, url: storyUrl.trim(), maxEpisodes: crawlEpisodeLimit })
      await loadProjectData(selected.id)
      await reloadProjects()
      setActiveScriptId(result.story.id)
      setStoryMedia(await window.contentFactory.storyMedia.get(selected.id))
      setView('scripts')
      setMessage(`✓ Đã crawl ${result.episodes.length} tập từ ${new URL(result.sourceUrl).hostname}. Chọn voice, tạo thumbnail rồi Generate ${result.episodes.length} Reel Videos.`)
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)) } finally { setCrawlGenerating(false); setBusy(false) }
  }
  ...
  async function generateStoryMp3() {
    ...
    try {
      ...
      setStoryMedia(media); await reloadProjects(); setMessage('✓ Bước 1/4 hoàn tất: Story MP3 đã sẵn sàng. Tiếp theo hãy chọn Video hoặc Ảnh background.')
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)) } finally { setBusy(false) }
  }
  ...
  async function generateThumbnail() {
    ...
    setBusy(true); setMessage(`Đang tạo thumbnail 16:9 bằng ${settings.provider}...`)
    try {
      const saved = await window.contentFactory.scripts.update({ scriptId: activeScript.id, title: activeScript.title ?? undefined, content: editorContent })
      const media = await window.contentFactory.storyMedia.generateThumbnail({ projectId: selected.id, scriptId: saved.id, prompt: thumbnailPrompt || undefined })
      setStoryMedia(media); setMessage('✓ Thumbnail đã tạo xong và được lưu vào thư mục images của project.')
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)) } finally { setBusy(false) }
  }
  ...
  async function chooseBackground(kind: BackgroundKind) {
    ...
    setBusy(true); setMessage(kind === 'IMAGE' ? 'Đang chọn ảnh background...' : 'Đang chọn background video...')
    try {
      const media = await window.contentFactory.storyMedia.chooseBackground(selected.id, kind)
      if (media) { setStoryMedia(media); setMessage(`✓ Bước 2/4 hoàn tất: ${kind === 'IMAGE' ? 'Ảnh' : 'Video'} background đã sẵn sàng. Chọn SFX rồi Generate Story Video.`) }
  ...
  async function renderStoryVideo() {
    ...
    setStoryVideoProgress({ current: 0, total: 1, percent: 0, stage: 'STARTING', message: videoFormat === 'REEL' ? 'Đang tính số video Short 9:16...' : 'Đang chuẩn bị Story video...' })
    setStoryVideoGenerating(true)
    setBusy(true); setMessage(videoFormat === 'REEL' ? 'Đang chia Story thành các Short 9:16 dài tối đa 3 phút...' : `Đang trộn ${soundEffect.preset.toLowerCase()} SFX ${soundEffect.volume}% và render video...`)
    try {
      const media = await window.contentFactory.storyMedia.render({ projectId: selected.id, format: videoFormat, fitMode, soundEffect })
      setStoryMedia(media); await reloadProjects()
      setMessage(videoFormat === 'REEL' ? `✓ Đã tạo ${media.storyVideoParts.length} video Short 9:16 liên tục, mỗi phần có SFX.` : `✓ Bước 4/4 hoàn tất: Story video đã render xong với ${soundEffect.preset.toLowerCase()} SFX ở mức ${soundEffect.volume}%.`)
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)) } finally { setStoryVideoGenerating(false); setBusy(false) }
  }
  ...
  async function generateReelVideos() {
    ...
    if (!storyMedia?.thumbnailPath) return setMessage('Hãy Generate Thumbnail Truyện trước; app sẽ thêm số TẬP cho từng Reel.')
    if (!storyMedia.backgroundPath) return setMessage('Hãy chọn Video hoặc Ảnh background trước.')
    setReelProgress({ current: 0, total: count, percent: 0, stage: 'STARTING', message: `Đang chuẩn bị ${count} tập...` })
    setReelGenerating(true)
    setBusy(true); setMessage(`Đang tạo ${count} MP3, thumbnail và Reel videos kèm SFX...`)
    try {
      const media = await window.contentFactory.storyMedia.generateReelVideos({ projectId: selected.id, fitMode, soundEffect })
      setStoryMedia(media); await reloadProjects(); setMessage(`✓ Đã tạo xong ${count} Reel videos, mỗi video có 1 SFX và thumbnail TẬP 1–${count}.`)
    } catch (error) { setMessage(error instanceof Error ? error.message : String(error)) } finally { setReelGenerating(false); setBusy(false) }
  }
  ...
  async function removeScript(script: ScriptDTO) {
    if (!selected) return
    const label = script.type === 'LONG_STORY' ? `Story v${script.version}` : (script.title ?? `Reel ${script.version}`)
    if (!window.confirm(`Xóa ${label}? Thao tác này không thể hoàn tác.`)) return
    setBusy(true); setMessage(`Đang xóa ${label}...`)
    try {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      <header className="topbar"><div><h1>{view === 'dashboard' ? 'Local Content Factory' : view === 'ideas' ? 'Idea Bank' : view === 'scripts' ? 'Story & Reels' : 'Settings'}</h1><p>{view === 'scripts' ? 'Selected Idea → Story → Review → Rewrite/Approve → Reels' : 'Local-first Facebook content workflow.'}</p></div><div className="topbar-actions">{selected && <button className="primary" onClick={() => void window.contentFactory.storyMedia.openOutput(selected.id)}>Mở output truyện</button>}<button className="secondary" onClick={() => void window.contentFactory.app.openStorageFolder()}>Mở storage nội bộ</button></div></header>
      {message && <div className="banner">{message}</div>}
      {busy && <div className="global-progress-panel" role="status" aria-live="polite">
        <div><strong>{crawlGenerating && crawlProgress ? `${crawlProgress.percent}%` : reelGenerating && reelProgress ? `${reelProgress.percent}%` : storyVideoGenerating && storyVideoProgress ? `${storyVideoProgress.percent}%` : `${estimatedProgress}%`}</strong><span>{crawlGenerating && crawlProgress ? crawlProgress.message : reelGenerating && reelProgress ? reelProgress.message : storyVideoGenerating && storyVideoProgress ? storyVideoProgress.message : `${message || 'Đang xử lý...'} · ước tính`}</span></div>
        <div className="global-progress-track">
          <i style={{ width: `${crawlGenerating && crawlProgress ? crawlProgress.percent : reelGenerating && reelProgress ? reelProgress.percent : storyVideoGenerating && storyVideoProgress ? storyVideoProgress.percent : estimatedProgress}%` }} />
        </div>
      </div>}
      ...
      <section className="card crawler-first-step">
        <div className="crawler-first-head">
          ...
          <div><h2>Crawl truyện theo từng chương</h2><p>Dán link truyện; hệ thống sẽ tìm danh sách chương, mở chi tiết từng chương và tạo mỗi chương thành một tập.</p></div>
          <span className="crawler-project-chip">{selected ? selected.name : 'Chưa chọn project'}</span>
        </div>
        {selected ? <>
          <div className="crawler-first-form">
            <label className="grow">Link trang danh sách / chi tiết truyện<input type="url" value={storyUrl} onChange={(e)=>setStoryUrl(e.target.value)} placeholder="https://website.com/ten-truyen" /></label>
            <label>Số chương tối đa<input type="number" min={1} max={100} value={crawlEpisodeLimit} onChange={(e)=>setCrawlEpisodeLimit(Math.min(100,Math.max(1,Number(e.target.value)||1)))} /></label>
            <button className="primary" onClick={()=>void crawlStory()} disabled={busy || !/^https?:\/\//i.test(storyUrl.trim())}>Tìm chương & crawl</button>
          </div>
          <p className="crawler-note">Trang tổng không được dùng làm tập. Mỗi link chương tạo đúng một script/video tập; xong sẽ tự chuyển sang bước Scripts.</p>
          {crawlProgress && <div className={`reel-progress ${crawlProgress.stage==='DONE'?'done':''}`}><div><strong>{crawlProgress.percent}%</strong><span>{crawlProgress.message}</span></div><div className="progress-track"><i style={{width:`${crawlProgress.percent}%`}} /></div></div>}
        </> : <div className="empty crawler-empty">Hãy tạo hoặc chọn project bên dưới trước khi crawl.</div>}
      </section>
      <section className="grid"><div className="card"><h2>Create project</h2><form onSubmit={createProject} className="form"><label>Project name<input value={name} onChange={(e) => setName(e.target.value)} required /></label><label>Topic<textarea value={topic} onChange={(e) => setTopic(e.target.value)} rows={4} /></label><label>Niche<select value={niche} onChange={(e) => setNiche(e.target.value)}><option value="family">Family</option><option value="life">Life</option><option value="love">Love</option><option value="knowledge">Knowledge</option></select></label><button className="primary" disabled={busy}>Create project</button></form></div>
      <div className="card project-card"><h2>Current project</h2>{selected ? <><div className="project-title">{selected.name}</div><div className="meta">{selected.niche ?? 'general'} · {selected.status}</div><div className="topic">{selected.topic ?? 'No topic'}</div><div className="steps">{['Project','Crawl / Ideas','Story','Reels','Media','Render'].map((step,index)=><div className="step" key={step}><span className={index < stepsDone ? 'step-dot done' : 'step-dot'} />{step}</div>)}</div><div className="inline-controls"><label>Ideas<input type="number" min={1} max={30} value={ideaCount} onChange={(e)=>setIdeaCount(Number(e.target.value))}/></label><button className="primary grow" onClick={generateIdeas} disabled={busy}>Generate ideas</button></div></> : <div className="empty">Chưa có project.</div>}</div></section>
      ...
      {activeScript.type === 'LONG_STORY' && <ThumbnailGenerator busy={busy} media={storyMedia} prompt={thumbnailPrompt} onPromptChange={setThumbnailPrompt} onGenerate={()=>void generateThumbnail()} />}
      {activeScript.type === 'LONG_STORY' && <><div className="rewrite-row"><input placeholder="Rewrite instruction / dùng feedback AI nếu để trống" value={rewriteNote} onChange={(e)=>setRewriteNote(e.target.value)}/><button className="secondary" onClick={rewriteStory} disabled={busy}>Rewrite → new version</button></div><div className="voice-gate"><div className="voice-title"><div><strong>Voice before approve</strong><span>{selected?.voiceName ? `Selected: ${selected.voiceName}` : 'Test và chọn giọng trước khi approve'}</span></div><button className="secondary" onClick={loadVoices} disabled={busy || (!isCapCut && !voiceSettings.hasElevenLabsKey)}>Load voices</button></div>{!isCapCut && !voiceSettings.hasElevenLabsKey && <p className="voice-warning">Chưa có ElevenLabs API key. Vào Settings → Voice/TTS.</p>}{voices.length > 0 && <><div className="voice-controls"><label>Voice<select value={voiceId} onChange={(e)=>{setVoiceId(e.target.value);setVoiceAudio('')}}>{voices.map(v=><option key={v.id} value={v.id}>{v.name}{v.labels.gender ? ` · ${v.labels.gender}` : ''}{v.labels.accent ? ` · ${v.labels.accent}` : ''}</option>)}</select></label><label>Test sentence<input value={voiceTestText} onChange={(e)=>setVoiceTestText(e.target.value)}/></label></div><div className="button-row voice-actions"><button className="secondary" onClick={testVoice} disabled={busy}>Test voice</button><button className="primary" onClick={useVoice} disabled={busy || !voiceId}>Use this voice</button></div>{voiceAudio && <audio className="voice-player" src={voiceAudio} controls autoPlay />}</>}</div><StoryMediaFlow busy={busy} ffmpegReady={Boolean(health?.ffmpeg)} hasVoice={Boolean(selected?.voiceId)} media={storyMedia} videoFormat={videoFormat} fitMode={fitMode} soundEffect={soundEffect} reelProgress={reelProgress} onGenerateAudio={()=>void generateStoryMp3()} onGenerateReelVideos={()=>void generateReelVideos()} onChooseBackground={(kind)=>void chooseBackground(kind)} onRender={()=>void renderStoryVideo()} onVideoFormatChange={setVideoFormat} onFitModeChange={setFitMode} onSoundEffectChange={setSoundEffect} /><div className="generate-reels"><label>Reels<input type="number" min={1} max={10} value={reelCount} onChange={(e)=>setReelCount(Number(e.target.value))}/></label><div className="button-row grow"><button className="primary" onClick={approveAndGenerateReels} disabled={busy || !selected?.voiceId}>Approve Version & Generate Reels</button><button className="secondary" onClick={generateReels} disabled={busy || !stories.some(s=>s.approved)}>Generate Again</button></div></div></>}</>}</div></section>}
